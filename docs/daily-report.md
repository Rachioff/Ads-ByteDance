...
**网络相关**：
- [ ] 网络超时：断开网络 → 启动 App → 验证 Mock 模式数据正常加载；AI 功能验证降级
- [ ] Mock 数据为空：清空 assets JSON → 验证 EmptyState 正确展示
- [ ] 图片加载失败：修改占位 URL → 验证错误占位图 + 重试机制（OkHttp 拦截器指数退避 3 次）

**交互边界**：
- [ ] 快速切换 Tab：连续点击 3 个频道 → 验证数据加载不混乱、无 crash
- [ ] 快速滑动：大幅 Fling 信息流 → 验证视频封面跳过加载（`isScrollInProgress` 检测）、列表无卡顿
- [ ] 快速点赞/取消：连续点击点赞按钮 → 验证计数不出现负数（`coerceAtLeast(0)`）
- [ ] 同时打开多个详情页：快速点击多张卡片 → 验证 Navigation 返回栈正常

**数据边界**：
- [ ] AI API 不可用：关闭微服务 → 验证 AI 摘要降级为静态内容
- [ ] Chat Bot 不可用：关闭微服务 → 验证对话搜索降级为本地关键词搜索
- [ ] 搜索无结果：输入不存在的内容 → 验证空态 UI
- [ ] 新用户（无行为数据）：首次启动 → 验证画像为空、精选频道默认热度排序

**视频播放边界**：
- [ ] 播放中滑动离开 → 验证播放器暂停 + 归还池
- [ ] 播放中切换 Tab → 验证播放器释放
- [ ] 快速滑动中视频可见性检测 → 验证 `isScrollInProgress=true` 时跳过

#### 2. 状态视图全覆盖确认

| 状态 | 场景 | 验证结果 |
|------|------|---------|
| **Loading** | 首次加载/下拉刷新/上拉加载/AI 生成中 | ✅ |
| **Empty** | 筛选无结果/搜索无结果/无行为数据 | ✅ |
| **Error** | 网络错误 + 重试按钮 | ✅ |
| **End** | "没有更多了" Footer | ✅ |
...
| 场景 | 操作 | 验证 |
|------|------|------|
| 详情页点赞 → 信息流 | 进入详情 → 点赞 → 返回 | 对应卡片 ♥ 已变红 + 计数 +1 |
| 详情页收藏 → 信息流 | 进入详情 → 收藏 → 返回 | 对应卡片 ⭐ 已变色 + 计数 +1 |
| 详情页分享 → 信息流 | 进入详情 → 分享 → 取消 | 对应卡片分享计数 +1 |
| 信息流点赞 → 详情页 | 信息流点赞 → 进入详情 | 详情页点赞状态已同步 |
| 信息流收藏 → 详情页 | 信息流收藏 → 进入详情 | 详情页收藏状态已同步 |
| 滚动位置保持 | 信息流滑动 → 进入详情 → 返回 | 返回后列表在原位置 |

#### 4. AI 降级方案验证

| 场景 | 操作 | 预期 | 验证 |
|------|------|------|------|
| AI API 超时 | 设置超时 1s，请求实际需 3s | 降级为静态摘要 + 标签 | ✅ |
| AI 返回非法 JSON | Mock 返回纯文本 | 解析失败 → 降级 | ✅ |
| AI 返回空 summary | Mock 返回 `{"summary":""}` | 降级为描述前 80 字 | ✅ |
| 微服务 503 | 关闭微服务 | 对话搜索显示降级提示 | ✅ |
| 网络断开 | 切断网络 → AI 请求 | 降级静态内容 | ✅ |

#### 5. 真机/模拟器功能回归

- 在 Android Emulator (API 34) 上跑全套功能流程
- 重点验证：ExoPlayer 播放、Crossfade 动画、Navigation 返回栈
- （可选）在真机上安装 APK 测试
...
|------|------|---------|
| REVIEW | 全模块 | 边界场景覆盖确认 |
| REVIEW | `feed/ui/component/FeedStates.kt` | 状态视图完整性确认 |
| REVIEW | `ai/api/AiResponseParser.kt` | 降级逻辑覆盖确认 |
| REVIEW | `search/viewmodel/SearchViewModel.kt` | 降级路径覆盖确认 |

### 文档更新

- `docs/daily-report.md`：Day 12 日报
...
```
Day 1-2   ▌ 基础搭建：Gradle + Koin + 模块骨架 + 数据模型 + Mock 数据 + Room
Day 3-5   ▌ 核心体验：Compose 信息流 3 种卡片 + 频道切换 + 视频播放器 + 详情页
Day 6-8   ▌ AI 增强：AI 摘要/标签 + 三级缓存 + 对话搜索 + 标签过滤 + 图片加载优化
Day 9-11  ▌ 数据闭环：行为采集 + 画像 + 个性化推荐 + 曝光统计 + 性能优化 + 稳定性
Day 12 ▌ 交付收尾：测试 + 边界 + 文档 + 视频
```

### 技术亮点

1. **Compose 精确重组体系**：`mutableStateMapOf` key 级粒度 + `@Stable`/`@Immutable` 注解 + `contentType` 布局分组
2. **AI 多层容错架构**：Prompt 约束 + 三层解析容错 + 三级缓存 + 两级降级
3. **VideoPlayer Crossfade + PlayerPool**：封面态 ↔ 播放态无缝切换 + 实例池化复用
4. **Repository 驱动的跨页面状态同步**：内存缓存作为真相源 + `interactionVersion` 版本驱动重组
5. **完整的问题追踪体系**：根因分析 → 方案对比 → 设计原则提炼 → 文档记录

### 后续改进方向

1. **分页数据持久化**：Room 中缓存已加载的广告列表，支持离线浏览
2. **视频自适应码率**：根据网络状况切换视频清晰度
3. **A/B 实验框架**：支持多套推荐策略在线对比
4. **用户反馈闭环**：增加"不感兴趣"、"举报"等负反馈机制
5. **CI/CD**：GitHub Actions 自动构建 + 测试 + APK 发布
6. **单元测试**：ViewModel + Repository 的单元测试覆盖（JUnit + MockK + Turbine）
